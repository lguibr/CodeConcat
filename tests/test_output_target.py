...
def test_determine_output_target_tty_fallback():
    ...
    with patch("sys.stdout.isatty", return_value=True), patch("codeconcat.main.logger"):
...
def test_determine_output_target_non_tty_fallback():
    ...
    with patch("sys.stdout.isatty", return_value=False), patch("codeconcat.main.logger"):
